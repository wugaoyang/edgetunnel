...
// 5.161.191.23
__publicField(AppParam, "defaultProxyIp", "5.161.191.23");
__publicField(AppParam, "proxyIP", _AppParam.defaultProxyIp);
__publicField(AppParam, "proxyIpMap", /* @__PURE__ */ new Map([["us", _AppParam.defaultProxyIp], ["jp", "45.32.25.171"], ["hk", "62.72.163.109"], ["kr", "144.24.78.162"]]));
...
var SubService = class {
   ...
   */
  static processVlessHeader(vlessBuffer, userID) {
   ...
   */
  static async handleTCPOutBound(remoteSocket, addressType, addressRemote, portRemote, rawClientData, webSocket, vlessResponseHeader, log) {
    ...
    async function connectAndWrite(address, port, socks = false) {
      ...
      const tcpSocket2 = socks ? await SubService.socks5Connect(addressType, address, port, log) : connect({
        // @ts-ignore
        hostname: address,
        port
      });
   ...
   * @param {function} log 日志记录函数
   */
  static async socks5Connect(addressType, addressRemote, portRemote, log) {
  ...
  static async sendMessage(type, ip, add_data = "") {
...
__name(initProxyIp, "initProxyIp");
...
__name(initSocks5Address, "initSocks5Address");
